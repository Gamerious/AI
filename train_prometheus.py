#!/usr/bin/env python3
"""
Prometheus AI - Full Training & Evaluation Pipeline

Trains the Prometheus architecture on 24 diverse tasks,
then tests autoregressive generation to verify real learning.
"""
...
def train(model, dataset, max_steps, lr=3e-4, warmup=200, batch_size=32, log_every=200):
...
def generate(model, prefix_tokens, max_new=10, seq_len=64, min_new=1):
    """Generate tokens autoregressively.

    Args:
        min_new: minimum tokens to generate before allowing EOS/PAD stop
    """
    model.eval()
    device = next(model.parameters()).device
    ...
    for _ in range(max_new):
        ...
        # If we haven't generated enough tokens, suppress EOS/PAD
...
def test_generation(model, config):
    print(f"\n{'='*70}")
    print(f"  PROMETHEUS GENERATION TESTS")
    print(f"{'='*70}")
    ...
    for name, prefix, expected in tests:
        gen = generate(model, prefix, max_new=len(expected)+2, seq_len=config.max_seq_len, min_new=len(expected))
        match = gen[:len(expected)] == expected
        if match:
            correct += 1
            print(f"  [OK] {name}")
        else:
            g = [t-OFFSET if t>=OFFSET else f"[{t}]" for t in gen[:len(expected)]]
...
def main():
    print("="*70)
    print("  PROMETHEUS AI - TRAINING & EVALUATION")
    print("="*70)
    ...
    config = PrometheusConfig(
        vocab_size=VOCAB_SIZE, d_model=192, n_heads=6, d_ff=384,
        max_seq_len=SEQ_LEN, max_depth=16, halt_threshold=0.95,
        n_memory_slots=256, memory_top_k=12, n_read_heads=3,
        n_pred_levels=4, n_settle_steps=4,
        neuron_sparsity=0.85, plastic_rank=24,
    )
    ...
    # Generate data
    print(f"\n  Generating training data (24 tasks)...")
    all_tasks = list(CombinedTaskDataset.TASK_CLASSES.keys())
    train_data = CombinedTaskDataset(n_samples_per_task=5000, seq_len=SEQ_LEN, vocab_size=VOCAB_SIZE, tasks=all_tasks)

    # Phase 1: Full multi-task training
    print(f"\n{'#'*70}")
    print(f"  PHASE 1: Multi-Task Training (8000 steps)")
    print(f"{'#'*70}")
    train(model, train_data, max_steps=8000, lr=3e-4, warmup=500, batch_size=32, log_every=500)
    score1 = test_generation(model, config)

    # Phase 2: Hard task fine-tuning with more data
    hard_tasks = ["addition", "subtraction", "multiplication", "fibonacci",
                  "sorting", "boolean_logic", "palindrome", "bracket_matching",
                  "arithmetic_sequence", "implication_chain", "token_counting", "associative_recall"]
    hard_data = CombinedTaskDataset(n_samples_per_task=8000, seq_len=SEQ_LEN, vocab_size=VOCAB_SIZE, tasks=hard_tasks)

    print(f"\n{'#'*70}")
    print(f"  PHASE 2: Hard Task Fine-Tuning (6000 steps)")
    print(f"{'#'*70}")
    train(model, hard_data, max_steps=6000, lr=1e-4, warmup=300, batch_size=32, log_every=500)
    score2 = test_generation(model, config)

    # Phase 3: Arithmetic focus (the weakest area)
    arith_tasks = ["addition", "subtraction", "multiplication", "fibonacci",
                   "arithmetic_sequence", "number_comparison", "sequence_sum"]
    arith_data = CombinedTaskDataset(n_samples_per_task=10000, seq_len=SEQ_LEN, vocab_size=VOCAB_SIZE, tasks=arith_tasks)

    print(f"\n{'#'*70}")
    print(f"  PHASE 3: Arithmetic Focus (4000 steps)")
    print(f"{'#'*70}")
    train(model, arith_data, max_steps=4000, lr=5e-5, warmup=200, batch_size=32, log_every=500)
    ...
    print(f"{'#'*70}")
    train(model, train_data, max_steps=3000, lr=2e-5, warmup=100, batch_size=32, log_every=500)
    score4 = test_generation(model, config)

    # Save
    torch.save({"model_state_dict": model.state_dict(), "config": config.to_dict()}, "prometheus_trained.pt")
    ...
    summary = evaluator.print_results(results, "PROMETHEUS FINAL EVALUATION")

    print(f"\n{'='*70}")
    print(f"  PROMETHEUS RESULTS")
    print(f"  Real params:      {params['total_params']:,}")
